...
const SignIn = () => {
  ...
  const referralCode = searchParams.get("ref") || sessionStorage.getItem("lumka_referral") || "";

  // Check if already logged in & send welcome email on first sign-up
  useEffect(() => {
    const { data: { subscription } } = supabase.auth.onAuthStateChange(async (event, session) => {
      if (event === "SIGNED_IN" && session?.user) {
        // Send welcome email (fire and forget)
        supabase.functions.invoke("waitlist-signup", {
    ...
    <div className="min-h-screen bg-background flex">
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6 py-12 overflow-y-auto">
        <div className="w-full max-w-md">
          ...
          {referralCode && (
            <div className="bg-mint/10 border border-mint/30 rounded-lg px-4 py-2.5 mb-6">
              <p className="font-body text-sm text-mint font-medium">🎉 You were referred! Sign up to give your friend a boost.</p>
            </div>
          )}
          ...
          <div className="grid grid-cols-2 gap-3 mb-6">
            <button
              ...
              className="flex items-center justify-center gap-2 border border-border rounded-lg py-2.5 px-4 font-body text-sm font-medium text-foreground hover:bg-secondary transition-colors"
            >
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {isRegister && (
              <div>
                ...
                {form.confirm && form.password !== form.confirm && (
                  <p className="font-body text-xs text-destructive mt-1">Passwords don't match</p>
                )}
              </div>
            )}
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-gradient-to-r from-primary to-accent text-primary-foreground font-body font-semibold py-3 rounded-lg hover:-translate-y-0.5 transition-transform shadow-md text-sm flex items-center justify-center gap-2 disabled:opacity-70"
            >
              {loading ? (
                <><Loader2 size={16} className="animate-spin" /> {isRegister ? "Creating..." : "Signing in..."}</>
              ) : (
